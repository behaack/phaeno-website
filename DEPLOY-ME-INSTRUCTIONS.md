...
- Production database: Supabase project `phaeno-website` through
...
Supabase, and the server keeps only API, file manager, and optional Caddy
services.
...
ssh root@178.156.175.151 "chmod +x /tmp/migrate && /tmp/migrate --connection 'SUPABASE_NPGSQL_CONNECTION_STRING'"
```

Use the Supabase Npgsql connection string from `ConnectionStrings:phaeno-website`
...
- Back up Supabase before risky schema or data changes.
